...
const App = (function () {
  ...
  function refreshScopeOptions() {
    ...
    [...S.data.sales, ...S.data.purchases, ...S.data.transactions, ...(S.data.orders || []), ...(S.data.settlements || [])].forEach((r) => {
      if (r.year) years.add(S.num(r.year));
      if (r.month) { const m = S.num(r.month); if (m >= 1 && m <= 12) months.add(m); }
    });
      ...
      <div class="flow">
        ...
        ${step(2, purch.length > 0, "매입 마감", "발주서 기준 · 매입처별",
          ...
          `<button class="btn" data-go="purchases">매입 보기</button>
           <button class="btn ghost" data-act="import-hometax">🏛️ 홈택스 계산서</button>
  ...
  function renderCS(main) {
    ...
    main.innerHTML = `
      ...
      <div class="card"><h3>새 C/S 등록</h3>
        ...
        <div style="display:flex;gap:8px;align-items:flex-end;margin-top:10px">
          <span style="flex:1.6"><label style="display:block;font-size:12px;color:var(--muted);margin-bottom:3px">C/S내용 (고객 요청·컴플레인)</label><input id="cs-complaint" placeholder="예: 받자마자 터져있었다며 환불 요청" style="width:100%;border:1px solid var(--line);border-radius:8px;padding:7px 9px"></span>
      ...
      <div class="table-wrap"><table class="grid">
        <thead><tr><th style="width:40px" class="num">#</th><th>일자</th><th>사업장</th><th>채널</th><th>주문자(받는분)</th><th>주문번호</th><th>유형</th><th>상품·내용</th><th style="min-width:240px">C/S내용(고객요청)</th><th>환불금액</th><th>상태</th><th style="min-width:170px">처리내용</th><th></th></tr></thead>
        <tbody>${sorted.map((c) => `<tr data-s="${esc([c.channel, c.recipient, c.orderNo, c.item, c.complaint, c.note, c.type].join(" ").toLowerCase())}">
          ...
          <td style="text-align:right;white-space:nowrap">${c.refundAmount > 0 ? won(S.num(c.refundAmount)) : ""}</td>
          ...
          `<tr><td colspan="13" class="empty">아직 C/S 건이 없어요. 위에서 등록하세요.</td></tr>`}
        </tbody></table></div>`;
    wire(main);
    const found = $("#cs-found", main);
    const renumber = (q) => {
      let n = 0;
      $$("tbody tr", main).forEach((tr) => {
        const cell = tr.querySelector(".cs-rn"); if (!cell) return; // 빈 안내행 제외
        if (tr.style.display === "none") { cell.textContent = ""; return; }
        cell.textContent = ++n;
      });
      if (found) found.textContent = q ? `검색 ${n}건 / 전체 ${sorted.length}건` : `전체 ${sorted.length}건`;
    };
    renumber("");
    ...
    if (sIn) sIn.addEventListener("input", () => {
      const q = sIn.value.trim().toLowerCase();
      $$("tbody tr", main).forEach((tr) => { if (tr.querySelector(".cs-rn")) tr.style.display = (!q || (tr.dataset.s || "").includes(q)) ? "" : "none"; });
      renumber(q);
    });
  ...
  function blankStoreReport() {
    return { sales: 0, purchase: 0, channels: [], vendors: [], csItems: [], bank: { inCnt: 0, inSum: 0, outCnt: 0, outSum: 0 }, memo: "" };
  }
      ...
      <table class="doc-table" style="width:auto;min-width:260px;margin-bottom:8px">
        ...
        <tbody>
          ${_csTypes.filter((t) => _csByType[t].count > 0).map((t) => `<tr>
            <td>${t}</td>
            <td class="n">${_csByType[t].count}</td>
            <td class="n">${_csTotal ? (_csByType[t].count / _csTotal * 100).toFixed(0) : 0}%</td>
            <td class="n">${_csByType[t].refund > 0 ? won(_csByType[t].refund) : "-"}</td>
          </tr>`).join("") || `<tr><td colspan="4" class="empty">C/S 데이터 없음</td></tr>`}
          <tr class="sum">
            <td>합계</td><td class="n">${_csTotal}건</td><td class="n">100%</td>
            <td class="n ${_csRefund > 0 ? "neg" : ""}">${_csRefund > 0 ? won(_csRefund) : "-"}</td>
          </tr>
        </tbody>
      </table>
      ${R.csItems.length > 0 ? `<table class="doc-table" style="width:100%;table-layout:fixed">
        <thead><tr><th style="width:80px">품목</th><th class="n" style="width:44px">건수</th><th class="n" style="width:44px">비중</th><th>사유</th></tr></thead>
        <tbody>${R.csItems.map((r, idx) => `<tr>
          ...
          <td><textarea class="mr-in" data-sec="csItems" data-i="${idx}" data-f="reason" rows="2" placeholder="특이사항" style="width:100%;border:1px solid var(--line);border-radius:5px;padding:3px 6px;font-size:11px;background:transparent;resize:vertical;line-height:1.4">${esc(r.reason || "")}</textarea></td>
        </tr>`).join("")}</tbody>
      </table>` : ""}
      `;
      ...
      <table class="doc-table" style="width:auto;min-width:260px;margin-bottom:8px">
        ...
        <tbody>
          ${_csTypes2.filter((t) => _csByType2[t].count > 0).map((t) => `<tr>
            <td>${t}</td>
            <td class="n">${_csByType2[t].count}</td>
            <td class="n">${_csTotal2 ? (_csByType2[t].count / _csTotal2 * 100).toFixed(0) : 0}%</td>
            <td class="n">${_csByType2[t].refund > 0 ? won(_csByType2[t].refund) : "-"}</td>
          </tr>`).join("") || `<tr><td colspan="4" class="empty">C/S 데이터 없음</td></tr>`}
          <tr class="sum">
            <td>합계</td><td class="n">${_csTotal2}건</td><td class="n">100%</td>
            <td class="n ${_csRefund2 > 0 ? "neg" : ""}">${_csRefund2 > 0 ? won(_csRefund2) : "-"}</td>
          </tr>
        </tbody>
      </table>
      ${_csTopItems2.length ? `<table class="doc-table" style="width:100%;table-layout:fixed">
        <thead><tr><th style="width:80px">품목</th><th class="n" style="width:44px">건수</th><th class="n" style="width:44px">비중</th><th>사유</th></tr></thead>
  ...
  function wire(main) {
    $$("[data-act]", main).forEach((b) => b.addEventListener("click", () => {
      ...
      else if (b.dataset.act === "import-hometax") Modals.importHometax();
